...
class ChartDrawingsStorage {
  ...
  static Future<Map<String, DrawingsBundle>> loadAllWithAlerts() async {
    ...
    for (final key in keys) {
      if (key.startsWith(_tlPrefix))
        symbols.add(key.substring(_tlPrefix.length));
      else if (key.startsWith(_hlPrefix))
        symbols.add(key.substring(_hlPrefix.length));
    }
    ...
    for (final sym in symbols) {
      ...
      final hasAny =
          bundle.trendLines.any((t) => t.hasAlert) ||
  ...
  // ─── Load ALL symbols with ALL drawings (alerts or not) ─
  static Future<Map<String, DrawingsBundle>> loadAll() async {
    final prefs = await SharedPreferences.getInstance();
    final keys = prefs.getKeys();
    final symbols = <String>{};
    ...
    for (final sym in symbols) {
      final bundle = _loadFromPrefs(prefs, sym);
      // Load ALL drawings, not just those with alerts
      final hasAny =
          bundle.trendLines.isNotEmpty || bundle.horizLines.isNotEmpty;
      if (hasAny) result[sym] = bundle;
    }
    return result;
  }
  ...
  static DrawingsBundle _loadFromPrefs(SharedPreferences prefs, String symbol) {
    ...
    if (tlStr != null && tlStr.isNotEmpty) {
      try {
        ...
        print('⚠️ chart TL parse error ($symbol): $e');
      }
    }
    ...
    if (hlStr != null && hlStr.isNotEmpty) {
      try {
        ...
        print('⚠️ chart HL parse error ($symbol): $e');
      }
    }
